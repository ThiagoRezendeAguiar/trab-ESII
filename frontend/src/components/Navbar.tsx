import { Button, Flex, Image, Stack, Text, useDisclosure, Drawer, DrawerOverlay, DrawerContent, DrawerBody, IconButton, useBreakpointValue } from '@chakra-ui/react';
        ...
        <Flex justifyContent="space-between" maxW="container.xl" alignContent="center" w="100%" >
          ...
          {!isMobile ? (
            <Flex alignItems="center" >
              <Button onClick={() => navigate('/')} bg="none" _hover={{ opacity: 0.8 }}>
                <FaHome size="28" color="orange" />
              </Button>
              <Button onClick={() => navigate('/cart')} bg="none" _hover={{ opacity: 0.8 }}>
                <FaShoppingCart size="27" color="orange" />
              </Button>
              <Button onClick={() => navigate('/profile')} bg="none" _hover={{ opacity: 0.8 }}>
